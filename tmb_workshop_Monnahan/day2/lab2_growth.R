...
sample.lengths <- function(Nfish, n.ages, logLinf.mean = log(50), logLinf.sigma = .1,
                           ...
                           t0 = 5, Ntime = 40) {
  sample.ages <- function(n.ages, t0, Ntime) {
    sample((t0 + 1):Ntime, size = n.ages, replace = FALSE)
  }
  ...
  dat <- ldply(1:Nfish, function(i) {
    cbind(fish = i, sample.vbgf(
      ages = sample.ages(n.ages, t0 = t0, Ntime = Ntime),
...
g + theme_bw()
data <- list(
  Nfish = N, Nobs = nrow(dat), loglengths = dat$loglengths,
...
df <- data.frame(
  fish = 1:N, par = rep(c("logLinf", "logK"), each = N),
  mles = mles, lwr = mles - 1.96 * ses,
  upr = mles + 1.96 * ses
)
ggplot(df, aes(x = fish, y = mles, ymin = lwr, ymax = upr)) +
  geom_pointrange() +
