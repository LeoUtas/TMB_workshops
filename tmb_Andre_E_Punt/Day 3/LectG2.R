...
BuildTree2 <- function(model, Theta, ru, ThetaMinus, ThetaPlus, rminus, rplus, u, vj, j, eps) {
  ...
  if (j == 0) {
    ...
    ThetaMinus <- Outs$ThetaMinus
    rminus <- Outs$rminus
    ThetaPlus <- Outs$ThetaPlus
    rplus <- Outs$rplus
    Thetadash <- Outs$Thetadash
    ndash <- Outs$Sdash
    Sdash <- Outs$Sdash
  ...
  for (Irep in 1:Nreps)
  {
    ...
    while (s == 1) {
      ...
      if (vj < 0) {
        ...
        ThetaMinus <- StepNeg$ThetaMinus
        rminus <- StepNeg$rminus
        Thetadash <- StepNeg$Thetadash
        Sdash <- StepNeg$Sdash
        ndash <- StepNeg$ndash
      }
      else {
        StepPos <- BuildTree2(model, ThetaPlus, rplus, ThetaMinus, ThetaPlus, rminus, rplus, uu, vj, j, eps)
        ThetaPlus <- StepPos$ThetaPlus
        rplus <- StepPos$rplus
        Thetadash <- StepPos$Thetadash
        Sdash <- StepPos$Sdash
        ndash <- StepPos$ndash
      }
      if (Sdash == 1) {
