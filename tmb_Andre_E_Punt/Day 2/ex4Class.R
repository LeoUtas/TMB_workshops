...
MCMCSumm <- function(file, best, Nyear, data, map, parameters) {
  ...
  for (Isim in 1:Nsim)
  {
    xx <- model$fn(post1a[Isim, ])
    Biomass[Isim, ] <- model$report()$BioPred[1:(Nyear + 1)]
  }
...
# Data vector
data <- list(
  Nyear = Nyear, Nclass = Nclass, Length = Length, Weight = Weight, X = X, S = S, SurveyS = SurveyS, M = M,
...
map <- list(
  LogRbar = factor(NA), LogNinit = rep(factor(NA), Nclass), LogFullF = rep(factor(NA), Nyear),
